...
::  Standalone-Activate-Local.cmd     Written by @abbodi1406 (MDL)
::  https://forums.mydigitallife.net/posts/1501441

::  With the great help from @RPO (MDL), Forked it to work with Multi
::  KMS Servers, Renewal Task, $OEM$ etc
:: -------------------------------------------------------------------
...
title [Fork]  Online KMS Activation - No Remnants
color 1F
...
reg query "%key%" /f Path /s | find /i "\Online_KMS_Activation_Script-Renewal" >nul && (set err=1)
reg query "%key%" /f Path /s | find /i "\Online_KMS_Activation_Script-Run_Once" >nul && (set err=1)
...
echo ------------------------------------------------------------------------
echo   Checking: 
echo   Scheduled Task[s] to renew the KMS Activation is installed.
echo   If you continue then all the things related to the Online KMS will be 
echo   removed after the activation.
echo ------------------------------------------------------------------------
echo.
...
if errorlevel 1 cls & goto continue
)
)
...
if defined error_ (
powershell write-host -back Black -fore Red Error - Try again.
) else (
echo Complete Uninstall was done successfully.
)
exit /b
...
set "_p=> nul ||"
:repeat
ping download.windowsupdate.com %_p% ping Bing.com %_p% ping 0x0.st %_p% ping baidu.com %_p% (
   if %loop%== %max_loop% (
...
echo Checking: Internet is connected.

::========================================================================================================================================
REM Servers_List - You can edit the Online KMS Servers in below line, Make sure to leave a space between the servers name.

set "servers=kms.srv.crsoo.com dimanyakms.sytes.net kms.digiboy.ir kms8.MSGuides.com kms9.MSGuides.com kms.library.hk kms.03k.org"
...
echo Activation wasn't successful.
echo Read the troubleshoot guide in ReadMe.txt
echo Exiting... &echo. &exit /b 1
)
...
echo Firewall app is not blocking the script Internet connection.
echo Read the troubleshoot guide in ReadMe.txt
echo Exiting... &echo. &exit /b 1
)
...
echo Checking: KMS Server ^(%KMS_IP%^) is online.
...
if %activation_ok%==0 (
echo --------------------------------------------------------------- &echo.
if not %online_server_count%==2 echo Activation wasn't successful. Trying another server... &echo.
set /a server_num+=1
goto :server
)
if defined Renewal_Task (echo Exiting...&echo. &exit /b 0)
if defined Run_Once (
echo Deleting Scheduled Task Online_KMS_Activation_Script-Run_Once
echo Deleting File %windir%\Online_KMS_Activation_Script\Online_KMS_Activation_Script-Run_Once.cmd
schtasks /delete /tn Online_KMS_Activation_Script-Run_Once /f 1>nul 2>nul
del /f /q %windir%\Online_KMS_Activation_Script\Online_KMS_Activation_Script-Run_Once.cmd >nul 2>&1
echo Exiting... &echo.
exit /b 0
)
exit /b
